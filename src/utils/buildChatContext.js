...
function flowLine(row) {
  ...
  return `${row.age}세: 총 ${row.total}만원/월, 연금 ${pensionOnly}만원/월, 임대 ${row.rental}만원/월, 국민연금 ${row.national}만원, DB ${row.db}만원, 세제비적격 ${row.nontax}만원`
}
...
- 분석 기간: 55세~90세
- DB퇴직연금: 61~80세, 잔액 ${Math.round((balances.db ?? 0) / 100_000 * 10) / 10}억원
