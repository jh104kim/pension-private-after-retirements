...
| 세제적격/퇴직연금성 | DB채권형·이율보증형(DB)·IRP·연금저축 | 퇴직소득세 또는 연금소득세 (추정) |
| 세제비적격 | 노후적립·인덱스Up·스마트Top | 10년 이상 유지 시 **비과세** |
| 자녀 양도 예정 | 우리아이변액연금 1·2 | 본인 연금 합계 제외, 별도 표기 |

### 분류 변경 메모

- 우리아이 1·2는 자녀에게 양도 예정이므로 `cashFlowByAge.total` 본인 합계에서 제외하고 `childTransfer`로 별도 표기한다.
- 이율보증형(`guaranteed`)은 퇴직연금 DB 운용상품으로 재분류하여 `dbTotal = db + guaranteed`에 포함한다.
- DB채권형은 DB 퇴직연금 내 채권 운용상품 또는 국고채 운용상품으로 추정하며, 정확한 계좌 성격은 회사/운용사 확인 필요로 표시한다.
...
연금저축(연 237만) + IRP 인출액 합산이 **1,500만원 초과 시 종합과세 전환** (2024년부터 1,200만→1,500만 상향).  
